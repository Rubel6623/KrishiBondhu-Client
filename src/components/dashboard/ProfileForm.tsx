...
import { User, Mail, Phone, MapPin, Camera, Loader2, ShieldCheck, CheckCircle2, ShieldAlert } from "lucide-react";
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[300px_1fr] gap-8">
        ...
        <div className="space-y-6">
          <div className="bg-card border border-border rounded-[32px] p-8 text-center shadow-sm">
            ...
            <p className="text-xs font-black uppercase tracking-[0.2em] text-muted-foreground mb-6">{role}</p>
            ...
            <div className="flex items-center justify-center gap-2 px-4 py-2 bg-green-50 dark:bg-green-brand/10 text-green-brand rounded-full border border-green-brand/20">
              ...
              <span className="text-xs font-bold uppercase tracking-wider">System Verified</span>
            </div>
          </div>

          <div className={`${role === 'ADMIN' ? 'bg-zinc-900' : 'bg-green-brand'} text-white rounded-[32px] p-6 shadow-lg`}>
            <h4 className="font-bold mb-4 flex items-center gap-2">
              {role === 'ADMIN' ? <ShieldAlert size={18} /> : <CheckCircle2 size={18} />}
              {role === 'ADMIN' ? "Admin Security" : "Profile Score: 98%"}
            </h4>
            <p className="text-xs text-white/80 leading-relaxed">
              {role === 'ADMIN' 
                ? "You have full access to platform settings. Keep your credentials secure and enable 2FA for maximum security."
                : "Your profile is nearly complete. Verifying your phone number increases your trust score among other users."}
            </p>
          </div>
        </div>
        ...
        <div className="bg-card border border-border rounded-[32px] p-8 md:p-10 shadow-sm relative overflow-hidden">
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-8 relative z-10">
            ...
            <div className="pt-4 border-t border-border flex justify-end gap-4">
              ...
              <Button
                type="submit"
                className={`rounded-xl px-10 h-12 ${role === 'ADMIN' ? 'bg-zinc-900 hover:bg-black' : 'bg-green-brand hover:bg-green-deep'} text-white font-black uppercase tracking-widest text-xs shadow-xl disabled:opacity-50 transition-all hover:-translate-y-1`}
                disabled={!isDirty || isLoading}
              >
                {isLoading ? (
